...
const Signup = () => {
    const { loading, isAuth, isRegistered } = useSelector((state) => state.user)
    ...
    useEffect(()=>{
        if(validateUserData) {
            navigate('/mon-compte/validation-de-compte', {state: validateUserData})
        }
    },[isRegistered])
                ...
                :
                    <div className="flex items-center justify-center xl:px-24 lg:px-16 md:px-8 px-4 lg:py-24 py-8">
                        <div className='flex flex-col gap-4 lg:w-1/2 w-full bg-[#F5F5F5] p-8'>
                            ...
                            <div>
                                <NavLink to={'/mon-compte/signin'} className='text-red-600 hover:underline'>Ou se connecter par ici</NavLink>
                            </div>
                        </div>
                    </div>
            }
